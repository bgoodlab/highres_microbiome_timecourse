...
for gene_name in target_genes:
    for var_type in passed_sites_map[gene_name]:
        ...
        gene_sites_map[gene_name] += passed_sites_map[gene_name][var_type]['sites'][0,1]
...
for gene_name in target_genes:
    ...
    for var_type in allele_counts_map[gene_name]:
        ...
        for snp_idx in xrange(0,len(locations)):
            ...
            if (f1>0.75) or (f0>0.75):
                
                within_locations.append((gene_number,location))
                within_freqs.append((f0,f1))
...
for snp_idx in xrange(0,len(within_locations)):
    print within_locations[snp_idx][1], within_genes[snp_idx], within_freqs[snp_idx][0], within_freqs[snp_idx][1], within_prevalences[snp_idx], within_targets[snp_idx]